...
For this example we are going to consider PTQ10, a low-cost electron donor polymer. As our approach includes running a rigid scan, only the linker group from side chains are considered, avoiding atomic overlapping and overestimated energy barriers. A priori, one should include all flexible single bonds highlighted in the figure below.

![tors_angles](images/tors_angles.png)

Our example will illustrate the procedure for fitting the torsional constants of the dihedral highlighted in cyan.

## Initial setup with DICEtools

The first part is adapted from [DICEtools tutorial](files/tutorial_dice.pdf) (unfortunately only in portuguese). From the topology and configuration files in the `.itp`, `.gro` and `.itp` formats, we start by generating the input files for DICE. If the topology file was generated using external tools, _e.g._, generated by the [LigParGen](https://traken.chem.yale.edu/ligpargen/) or [PolyParGen](http://polypargen.com), we start by running the `reorder_ligpargen.py` to avoid having different ordering of atoms among the 3 files.
...
The `tri_chelpg_org.log` is the Gaussian output file from a single-point calculation using ChelpG to compute the partial charges, and the code parses the partial charges making sure that the total charge is zero in the `parsed_reordered_trimer_org.itp` file. To avoid using the wrong topology file, I recommend deleting or moving the other `.itp` files, such that:
...
Is crucial to add the `-g` and `-c` flags because they will ensure that bonds and angles are extracted from the optimized geometry instead of default OPLS-AA values. Furthermore, the `--flexible-fragments` will write additional information to run simulations with flexible molecules.  

With the DICE input files, we can generate the input files for running the rigid torsional scan with Gaussian using the `plot_eff_tors.py` script:
...
In addition to the `.txt` and `.dfr` files generated above, one need to specify 4 atom numbers that define the dihedral angle. For the cyan angle, such atoms are indexed as `30`, `29`, `52` and `61` (this order was extracted from the proper dihedrals in the `.itp` file, but is not a requirement). The next number, `36` (`npoints`), corresponds to the amount of configurations included in the snapshot, such that the angle variation (in degrees) between two subsequent configurations is $\varphi=360/\text{npoints}$. The header of the Gaussian input file with the calculation details is written in the `top_g16.txt`:
...
**_PS:_** One can also add the `--gaussbot` flag to include a footer in the input files, _e.g._, provide custom solvent variables.  
...
Even though the dihedral energy is well fitted (bottom panel), the total energy end up presenting several non-physical fluctuations (top panel). Additionally, when checking the fitted constants we can easily identify a multicollinearity problem:
...
To deal with it, we can introduce a penalization term on the minimization function that considers the module of each coefficient. For example, in the case of Ridge regression, one seeks to minimize,

$$\mathcal{L}=\frac{||y - Xw||^2_2}{2N}  + \alpha ||w||_1 \ ,$$

where $N$ is the number of samples, $||*||_1$ is the 1-norm, $||*||_2$ the euclidean norm and $\alpha$ the penalization coefficient. By adding the `-m lassocv` flag, one can use `scikit-learn` implementation of Lasso regression with cross validation to optimize the fitting procedure:
...
However, the fitting is still not sufficiently accurate, specially for the describing the minimum and maximum points:

![lr2](images/linear_regression2.png)

Different approaches can be followed to improve this result. One can test different regression methods (_e.g._ Ridge), varying the penalization parameter ($\alpha$) or even include the weight of minimum points (run `-h` flag for more details). At the same time, we can select a set of points to consider in the calculation, which tend to avoid the overfitting and improve the results. For this case, we can get a very good agreement between energy profiles by running:
