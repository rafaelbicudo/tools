...
$$E_{RB}=\sum_{n=0}^5C_n[\cos{(\phi+\delta_n)}]^n\equiv\frac{1}{2}\sum_{n=1}^4F_n[1+(-1)^{n+1}\cos{(n\phi+\delta_n)}] \ .$$
...
**_PS:_** We recommend using [jmol](https://jmol.sourceforge.net) or other visualization tool to get the correct atom indexes of the desirable dihedral.  
